defmodule Kayrock.Client.CompressionTest do
  ...
  describe "Produce API & Fetch API with compression" do
    ...
    # RecordBatch format (v2-v5) supports gzip, snappy, and lz4
    ...
    # Produce API v7+ supports zstd compression (requires KAFKA_2_1_IV0+)
    for {compression, compression_num} <- [{"gzip", 1}, {"snappy", 2}, {"lz4", 3}, {"zstd", 4}] do
      for version <- [7, 8] do
        test "v#{version} - produce and reads data with compression: #{compression}", %{
          kafka: kafka
        } do
