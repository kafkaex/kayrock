defmodule Kayrock.RenewDelegationTokenTest do
  @moduledoc """
  Tests for RenewDelegationToken API (V0-V1).
  ...
  alias Kayrock.Test.Factories.RenewDelegationTokenFactory
  ...
  describe "versions compatibility" do
    for version <- 0..1 do
      test "serializes version #{version} request to expected binary" do
        ...
        {request, expected_binary} = RenewDelegationTokenFactory.request_data(version)
      ...
      test "deserializes version #{version} response to expected struct" do
        version = unquote(version)
        {response_binary, expected_struct} = RenewDelegationTokenFactory.response_data(version)

        response_module = Module.concat([Kayrock.RenewDelegationToken, :"V#{version}", Response])
    ...
    test "all available versions have modules" do
      for version <- 0..1 do
        request_module =
          Module.concat([Kayrock, RenewDelegationToken, :"V#{version}", Request])

        response_module =
          Module.concat([Kayrock, RenewDelegationToken, :"V#{version}", Response])
  ...
  describe "V0 - basic renew delegation token" do
    alias Kayrock.RenewDelegationToken.V0.Request
    alias Kayrock.RenewDelegationToken.V0.Response
    ...
    test "response deserializes correctly" do
      {response_binary, expected} = RenewDelegationTokenFactory.response_data(0)
      {response, <<>>} = Response.deserialize(response_binary)

      assert response == expected
      assert response.error_code == 0
      assert response.expiry_timestamp_ms == 1_705_086_400_000
      assert response.throttle_time_ms == 0
    end
  end

  describe "V1 - same schema as V0" do
    alias Kayrock.RenewDelegationToken.V1.Response

    test "response with throttle_time_ms deserializes correctly" do
      {response_binary, expected} = RenewDelegationTokenFactory.response_data(1)
      {response, <<>>} = Response.deserialize(response_binary)
  ...
  describe "integration with Request protocol" do
    test "api_vsn returns correct version for all versions" do
      for version <- 0..1 do
        module = Module.concat([Kayrock, RenewDelegationToken, :"V#{version}", Request])
  ...
  describe "truncated binary handling" do
    for version <- 0..1 do
      test "V#{version} response handles truncated binary" do
        version = unquote(version)
        {response_binary, _} = RenewDelegationTokenFactory.response_data(version)
        response_module = Module.concat([Kayrock.RenewDelegationToken, :"V#{version}", Response])
  ...
  describe "error code handling" do
    @error_codes [
      {0, 61, "DELEGATION_TOKEN_NOT_FOUND"},
      {1, 62, "DELEGATION_TOKEN_OWNER_MISMATCH"}
    ]
    ...
    for {version, error_code, name} <- @error_codes do
      test "V#{version} handles #{name} (#{error_code})" do
        ...
        response_binary =
          RenewDelegationTokenFactory.error_response(version, error_code: error_code)
