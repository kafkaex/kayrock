defmodule Kayrock.EndTxnTest do
  @moduledoc """
  Tests for EndTxn API (V0-V1).
  ...
  alias Kayrock.Test.Factories.EndTxnFactory
  ...
  describe "versions compatibility" do
    for version <- 0..1 do
      test "serializes version #{version} request to expected binary" do
        ...
        {request, expected_binary} = EndTxnFactory.request_data(version)
      ...
      test "deserializes version #{version} response to expected struct" do
        version = unquote(version)
        {response_binary, expected_struct} = EndTxnFactory.response_data(version)

        response_module = Module.concat([Kayrock.EndTxn, :"V#{version}", Response])
    ...
    test "all available versions have modules" do
      for version <- 0..1 do
        request_module = Module.concat([Kayrock, EndTxn, :"V#{version}", Request])
        response_module = Module.concat([Kayrock, EndTxn, :"V#{version}", Response])
  ...
  describe "V0 - basic end transaction" do
    alias Kayrock.EndTxn.V0.Request
    alias Kayrock.EndTxn.V0.Response

    test "request with commit serializes correctly" do
      request = %Request{
        correlation_id: 1,
        client_id: "test",
    ...
    test "request with abort serializes correctly" do
      request = %Request{
        correlation_id: 2,
        client_id: "test",
        transactional_id: "my-txn",
    ...
    test "response deserializes correctly" do
      {response_binary, expected} = EndTxnFactory.response_data(0)
  ...
  describe "V1 - same schema as V0" do
    alias Kayrock.EndTxn.V1.Response

    test "response with throttle_time_ms deserializes correctly" do
      {response_binary, expected} = EndTxnFactory.response_data(1)
      {response, <<>>} = Response.deserialize(response_binary)
  ...
  describe "integration with Request protocol" do
    test "api_vsn returns correct version for all versions" do
      for version <- 0..1 do
        module = Module.concat([Kayrock, EndTxn, :"V#{version}", Request])
  ...
  describe "truncated binary handling" do
    for version <- 0..1 do
      test "V#{version} response handles truncated binary" do
        version = unquote(version)
        {response_binary, _} = EndTxnFactory.response_data(version)
        response_module = Module.concat([Kayrock.EndTxn, :"V#{version}", Response])
  ...
  describe "malformed response handling" do
    ...
    test "V0 partial response (missing error_code) fails with FunctionClauseError" do
      ...
      assert_raise FunctionClauseError, fn ->
        Kayrock.EndTxn.V0.Response.deserialize(partial)
      end
    end
  end
  ...
  describe "error code handling" do
    ...
    for {version, error_code, name} <- @error_codes do
      test "V#{version} handles #{name} (#{error_code})" do
        ...
        response_binary = EndTxnFactory.error_response(version, error_code: error_code)
