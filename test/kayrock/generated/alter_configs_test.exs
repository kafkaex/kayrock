defmodule Kayrock.AlterConfigsTest do
  @moduledoc """
  Tests for AlterConfigs API (V0-V1).
  ...
  alias Kayrock.Test.Factories.AlterConfigsFactory
  ...
  describe "versions compatibility" do
    for version <- 0..1 do
      test "serializes version #{version} request to expected binary" do
        ...
        {request, expected_binary} = AlterConfigsFactory.request_data(version)
      ...
      test "deserializes version #{version} response to expected struct" do
        version = unquote(version)
        {response_binary, expected_struct} = AlterConfigsFactory.response_data(version)

        response_module = Module.concat([Kayrock.AlterConfigs, :"V#{version}", Response])
    ...
    test "all available versions have modules" do
      for version <- 0..1 do
        request_module = Module.concat([Kayrock, AlterConfigs, :"V#{version}", Request])
        response_module = Module.concat([Kayrock, AlterConfigs, :"V#{version}", Response])
  ...
  describe "V0 - basic alter configs" do
    alias Kayrock.AlterConfigs.V0.Request
    alias Kayrock.AlterConfigs.V0.Response

    test "request with topic config serializes correctly" do
      request = %Request{
        correlation_id: 1,
        client_id: "test",
        resources: [
    ...
    test "response deserializes correctly" do
      {response_binary, expected} = AlterConfigsFactory.response_data(0)
  ...
  describe "V1 - same schema as V0" do
    alias Kayrock.AlterConfigs.V1.Response

    test "response with throttle_time_ms deserializes correctly" do
      {response_binary, expected} = AlterConfigsFactory.response_data(1)
      {response, <<>>} = Response.deserialize(response_binary)
  ...
  describe "integration with Request protocol" do
    test "api_vsn returns correct version for all versions" do
      for version <- 0..1 do
        module = Module.concat([Kayrock, AlterConfigs, :"V#{version}", Request])
  ...
  describe "truncated binary handling" do
    for version <- 0..1 do
      test "V#{version} response handles truncated binary" do
        version = unquote(version)
        {response_binary, _} = AlterConfigsFactory.response_data(version)
        response_module = Module.concat([Kayrock.AlterConfigs, :"V#{version}", Response])
  ...
  describe "error code handling" do
    ...
    for {version, error_code, name} <- @error_codes do
      test "V#{version} handles #{name} (#{error_code})" do
        ...
        response_binary = AlterConfigsFactory.error_response(version, error_code: error_code)
        {response, <<>>} = response_module.deserialize(response_binary)

        [resource] = response.resources
        assert resource.error_code == error_code
        assert resource.error_message == "Configuration error"
      end
    end
  end
end
