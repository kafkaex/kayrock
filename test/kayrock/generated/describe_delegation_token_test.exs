defmodule Kayrock.DescribeDelegationTokenTest do
  @moduledoc """
  Tests for DescribeDelegationToken API (V0-V1).
  ...
  alias Kayrock.Test.Factories.DescribeDelegationTokenFactory
  ...
  describe "versions compatibility" do
    for version <- 0..1 do
      test "serializes version #{version} request to expected binary" do
        ...
        {request, expected_binary} = DescribeDelegationTokenFactory.request_data(version)
      ...
      test "deserializes version #{version} response to expected struct" do
        version = unquote(version)
        {response_binary, expected_struct} = DescribeDelegationTokenFactory.response_data(version)

        response_module =
          Module.concat([Kayrock.DescribeDelegationToken, :"V#{version}", Response])
        ...
        assert is_list(actual_struct.tokens)
      end
    end
    ...
    test "all available versions have modules" do
      for version <- 0..1 do
        request_module =
          Module.concat([Kayrock, DescribeDelegationToken, :"V#{version}", Request])
  ...
  describe "V0 - basic describe delegation token" do
    alias Kayrock.DescribeDelegationToken.V0.Request
    alias Kayrock.DescribeDelegationToken.V0.Response
    ...
    test "response deserializes correctly" do
      {response_binary, expected} = DescribeDelegationTokenFactory.response_data(0)
      ...
      assert token.principal_type == "User"
      assert token.principal_name == "admin"
      assert token.token_id == "token12345"
      assert is_binary(token.hmac)
      assert length(token.renewers) == 1
    end
  end

  describe "V1 - same schema as V0" do
    alias Kayrock.DescribeDelegationToken.V1.Response

    test "response with throttle_time_ms deserializes correctly" do
      {response_binary, expected} = DescribeDelegationTokenFactory.response_data(1)
      {response, <<>>} = Response.deserialize(response_binary)
  ...
  describe "integration with Request protocol" do
    test "api_vsn returns correct version for all versions" do
      for version <- 0..1 do
        module = Module.concat([Kayrock, DescribeDelegationToken, :"V#{version}", Request])
  ...
  describe "truncated binary handling" do
    for version <- 0..1 do
      test "V#{version} response handles truncated binary" do
        version = unquote(version)
        {response_binary, _} = DescribeDelegationTokenFactory.response_data(version)
  ...
  describe "error code handling" do
    @error_codes [
      {0, 59, "DELEGATION_TOKEN_EXPIRED"},
      {1, 60, "DELEGATION_TOKEN_AUTH_DISABLED"}
    ]
    ...
    for {version, error_code, name} <- @error_codes do
      test "V#{version} handles #{name} (#{error_code})" do
        ...
        response_binary =
          DescribeDelegationTokenFactory.error_response(version, error_code: error_code)
