defmodule Kayrock.IncrementalAlterConfigsTest do
  @moduledoc """
  Tests for IncrementalAlterConfigs API (V0-V1).
  ...
  alias Kayrock.Test.Factories.IncrementalAlterConfigsFactory
  ...
  describe "versions compatibility" do
    for version <- 0..1 do
      test "serializes version #{version} request to expected binary" do
        ...
        {request, expected_binary} = IncrementalAlterConfigsFactory.request_data(version)
      ...
      test "deserializes version #{version} response to expected struct" do
        version = unquote(version)
        {response_binary, expected_struct} = IncrementalAlterConfigsFactory.response_data(version)

        response_module =
          Module.concat([Kayrock.IncrementalAlterConfigs, :"V#{version}", Response])
    ...
    test "all available versions have modules" do
      for version <- 0..1 do
        request_module =
          Module.concat([Kayrock, IncrementalAlterConfigs, :"V#{version}", Request])
  ...
  describe "V0 - basic incremental alter configs" do
    alias Kayrock.IncrementalAlterConfigs.V0.Request
    alias Kayrock.IncrementalAlterConfigs.V0.Response
      ...
      request = %Request{
        ...
        resources: [
          %{
            ...
            configs: [
              %{name: "retention.ms", config_operation: 0, value: "86400000"}
            ]
          }
        ],
        validate_only: false
    ...
    test "response deserializes correctly" do
      {response_binary, expected} = IncrementalAlterConfigsFactory.response_data(0)
      ...
      assert resp.resource_name == "topic1"
    end
  end

  describe "V1 - same schema as V0" do
    alias Kayrock.IncrementalAlterConfigs.V1.Response

    test "response with throttle_time_ms deserializes correctly" do
      {response_binary, expected} = IncrementalAlterConfigsFactory.response_data(1)
      {response, <<>>} = Response.deserialize(response_binary)
  ...
  describe "integration with Request protocol" do
    test "api_vsn returns correct version for all versions" do
      for version <- 0..1 do
        module = Module.concat([Kayrock, IncrementalAlterConfigs, :"V#{version}", Request])
  ...
  describe "truncated binary handling" do
    for version <- 0..1 do
      test "V#{version} response handles truncated binary" do
        version = unquote(version)
        {response_binary, _} = IncrementalAlterConfigsFactory.response_data(version)
  ...
  describe "error code handling" do
    ...
    for {version, error_code, name} <- @error_codes do
      test "V#{version} handles #{name} (#{error_code})" do
        ...
        response_binary =
          IncrementalAlterConfigsFactory.error_response(version, error_code: error_code)

        {response, <<>>} = response_module.deserialize(response_binary)

        [resp] = response.responses
        assert resp.error_code == error_code
        assert resp.error_message == "Invalid config"
      end
    end
  end
end
