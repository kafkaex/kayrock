              ...
              [
                ...
                for v <- vals do
                  [
                    serialize(:int8, Map.fetch!(v, :resource_type)),
                    serialize(:nullable_string, Map.fetch!(v, :resource_name)),
                    serialize(:int8, Map.fetch!(v, :resource_pattern_type_filter)),
                    serialize(:nullable_string, Map.fetch!(v, :principal)),
                    serialize(:nullable_string, Map.fetch!(v, :host)),
                    serialize(:int8, Map.fetch!(v, :operation)),
                    serialize(:int8, Map.fetch!(v, :permission_type))
...
"
    ...
    defp deserialize_field(:matching_acls, :resource_pattern_type, acc, data) do
      {val, rest} = deserialize(:int8, data)

      deserialize_field(
        :matching_acls,
        :principal,
        Map.put(acc, :resource_pattern_type, val),
        rest
      )
    end
  ...
  (
    ...
    @type request_t :: Kayrock.DeleteAcls.V1.Request.t() | Kayrock.DeleteAcls.V0.Request.t()
  )

  (
    @typedoc "Union type for all response structs for this API"
    @type response_t :: Kayrock.DeleteAcls.V1.Response.t() | Kayrock.DeleteAcls.V0.Response.t()
  )
