...
"
              ...
              %{
                ...
                partition_responses: [
                  %{
                    ...
                    log_start_offset: nil | integer(),
                    record_errors: [
                      %{batch_index: nil | integer(), batch_index_error_message: nil | binary()}
                    ],
    ...
    defp deserialize_field(:record_errors, :batch_index, acc, data) do
      {val, rest} = deserialize(:int32, data)

      deserialize_field(
        :record_errors,
        :batch_index_error_message,
        Map.put(acc, :batch_index, val),
        rest
      )
    end

    defp deserialize_field(:record_errors, :batch_index_error_message, acc, data) do
      {val, rest} = deserialize(:nullable_string, data)
      deserialize_field(:record_errors, nil, Map.put(acc, :batch_index_error_message, val), rest)
    end

    defp deserialize_field(:partition_responses, :record_errors, acc, data) do
      <<num_elements::32-signed, rest::binary>> = data

      {vals, rest} =
        if num_elements > 0 do
          Enum.reduce(1..num_elements, {[], rest}, fn _ix, {acc, d} ->
            {val, r} = deserialize_field(:record_errors, :batch_index, %{}, d)
            {[val | acc], r}
          end)
        else
          {[], rest}
      ...
      deserialize_field(
        :partition_responses,
        :error_message,
        Map.put(acc, :record_errors, Enum.reverse(vals)),
        rest
      )
    end

    defp deserialize_field(:partition_responses, :error_message, acc, data) do
      {val, rest} = deserialize(:nullable_string, data)
      deserialize_field(:partition_responses, nil, Map.put(acc, :error_message, val), rest)
    end
  ...
  (
    ...
    @type request_t ::
            Kayrock.Produce.V8.Request.t()
            | Kayrock.Produce.V7.Request.t()
            | Kayrock.Produce.V6.Request.t()
  ...
  (
    ...
    @type response_t ::
            Kayrock.Produce.V8.Response.t()
            | Kayrock.Produce.V7.Response.t()
            | Kayrock.Produce.V6.Response.t()
