...
"
  ...
  defmodule V0.Response do
    @vsn 0
    @api :sasl_handshake
    @schema error_code: :int16, mechanisms: {:array, :string}
...
"
    _ = " THIS CODE IS GENERATED BY KAYROCK"
    defstruct error_code: nil, mechanisms: [], correlation_id: nil
    @typedoc "Response struct for the Kafka `#{@api}` API v#{@vsn}
"
    @type t :: %__MODULE__{
            error_code: nil | integer(),
            mechanisms: [nil | binary()],
    ...
    defp deserialize_field(:root, :error_code, acc, data) do
      ...
      deserialize_field(:root, :mechanisms, Map.put(acc, :error_code, val), rest)
    end

    defp deserialize_field(:root, :mechanisms, acc, data) do
      {val, rest} = deserialize_array(:string, data)
      deserialize_field(:root, nil, Map.put(acc, :mechanisms, Enum.reverse(val)), rest)
    end
