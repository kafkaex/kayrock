defmodule Kayrock.Client.ClusterMetadata do
  @moduledoc """
  ...
  alias Kayrock.Client.Broker
  alias Kayrock.Client.Topic
  ...
  def select_node(%__MODULE__{controller_id: controller_id} = cluster_metadata, :controller) do
  ...
  def select_node(%__MODULE__{} = cluster_metadata, {:topic_partition, topic, partition}) do
  ...
  def select_node(%__MODULE__{} = cluster_metadata, node_id) when is_integer(node_id) do
    ...
    new_brokers =
      Enum.into(new_cluster_metadata.brokers, %{}, fn {node_id, new_broker} ->
        case Map.get(old_brokers, node_id) do
          %Broker{pid: pid} when is_pid(pid) -> {node_id, %{new_broker | pid: pid}}
  ...
  def get_and_update_broker(%__MODULE__{brokers: brokers} = cluster_metadata, node_id, cb)
      when is_function(cb, 1) do
    broker = broker_by_node_id(cluster_metadata, node_id)
    {val, updated_broker} = cb.(broker)
    {val, %{cluster_metadata | brokers: Map.put(brokers, node_id, updated_broker)}}
  end
end
